...
public:
  ...
  Matrix(size_t nrow, size_t ncol)
      : m_nrow(nrow), m_ncol(ncol), m_buffer(vector<double, MyAllocator<double>>(m_nrow * ncol, 0))
  {
  } // origin constructor
...
template <class T> size_t MyAllocator<T>::allocate_byte = 0;
template <class T> size_t MyAllocator<T>::deallocate_byte = 0;
template <class T> size_t MyAllocator<T>::_bytes = 0;
...
PYBIND11_MODULE(_matrix, m)
{
  ...
  pybind11::class_<Matrix>(m, "Matrix")
      ...
      .def("__setitem__",
           [](Matrix &self, std::pair<size_t, size_t> idx, double val) { self(idx.first, idx.second) = val; })

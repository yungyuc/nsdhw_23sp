...
public:
    ...
    //In order to get the bytes of the matrix values.
    double operator()(size_t row, size_t col) const
    {
        return m_buffer[index(row, col)];
    }
    
    //In order to get the bytes of the matrix values by address.
    double &operator()(size_t row, size_t col)
    {
        return m_buffer[index(row, col)];
    }

    //transfer matrix value
...
//if matrix1==matrix2; then return true; else return false
...
//matrix multiplication mkl acceleration.
